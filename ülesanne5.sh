#!/bin/bash
# Skript, mis võtab failist kasutajad ja genereerib neile parooli. seejärel loob faili, kus sees kasutajad ja paroolid
if [ $# -ne 1 ];then # 1 tähistab siin parameetrite arvu, mida kasutatakse skriptis
		...
		paste -d":" $nimed pas.txt > kaspas.txt #käsen skriptil teha fail, kus on kirjas kasutajanimed ja paroolid
                done
        else
